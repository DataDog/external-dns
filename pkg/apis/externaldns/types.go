...
type Config struct {
	...
	AWSMaxResourceRecordsPerResourceRecordSet int
...
var defaultConfig = &Config{
	...
	AWSMaxResourceRecordsPerResourceRecordSet: 400,
...
func (cfg *Config) ParseFlags(args []string) error {
	...
	app.Flag("aws-max-resource-records-per-resource-record-set", "When using the AWS provider, truncate resource record that can be included in a record set to this number").Default(strconv.Itoa(defaultConfig.AWSMaxResourceRecordsPerResourceRecordSet)).IntVar(&cfg.AWSMaxResourceRecordsPerResourceRecordSet)
